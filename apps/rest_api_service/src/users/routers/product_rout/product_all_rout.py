...
async def get_all_product(
    ...
    try:
        products = await  product_service.get_all_products_paginated()
        safe_products = products if products is not None else[]

        return   safe_products
    except Exception as e:
        logger.error(
            f"Помилка при отриманні продуктів користувача  : {str(e)}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Внутрішня помилка сервера при отриманні списку продуктів."
        )